...
class P636 {
    fun exclusiveTime(n: Int, logs: List<String>): IntArray {
        ...
        for (log in logs) {
            val split = log.split(":")
            val functionId = split[0].toInt()
            val isStart = split[1] == "start"
            val timeStamp = split[2].toInt()

            if (isStart) {
                stack.push(functionId to timeStamp)
            } else {
                val (id, startTimeStamp) = stack.pop()
                val time = timeStamp - startTimeStamp - spendTime
                result[id] = time
                spendTime += time
            }
        }
